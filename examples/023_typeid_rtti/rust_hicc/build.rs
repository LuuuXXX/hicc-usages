...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find typeid_rtti.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("typeid_rtti_hicc");

    // Link the externally-built C++ static library (../cpp/build/libtypeid_rtti.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libtypeid_rtti.a");
