...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find class_volatile.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("class_volatile_hicc");

    // Link the externally-built C++ static library (../cpp/build/libclass_volatile.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libclass_volatile.a");
