...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find virtual_override.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("virtual_override_hicc");

    // Link the externally-built C++ static library (../cpp/build/libvirtual_override.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libvirtual_override.a");
