...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find std_function.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("std_function_hicc");

    // Link the externally-built C++ static library (../cpp/build/libstd_function.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libstd_function.a");
