...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find inline_functions.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("inline_functions_hicc");

    // Link the externally-built C++ static library (../cpp/build/libinline_functions.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libinline_functions.a");
