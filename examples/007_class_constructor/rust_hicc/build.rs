...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find class_constructor.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("class_constructor_hicc");

    // Link the externally-built C++ static library (../cpp/build/libclass_constructor.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libclass_constructor.a");
