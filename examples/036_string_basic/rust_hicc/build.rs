...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find string_basic.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("string_basic_hicc");

    // Link the externally-built C++ static library (../cpp/build/libstring_basic.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libstring_basic.a");
