...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find custom_deleter.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("custom_deleter_hicc");

    // Link the externally-built C++ static library (../cpp/build/libcustom_deleter.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libcustom_deleter.a");
