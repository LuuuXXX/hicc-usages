...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find mutable_member.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("mutable_member_hicc");

    // Link the externally-built C++ static library (../cpp/build/libmutable_member.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libmutable_member.a");
