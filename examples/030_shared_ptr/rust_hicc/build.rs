...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find shared_ptr.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("shared_ptr_hicc");

    // Link the externally-built C++ static library (../cpp/build/libshared_ptr.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libshared_ptr.a");
