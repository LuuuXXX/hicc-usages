...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find unique_ptr.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("unique_ptr_hicc");

    // Link the externally-built C++ static library (../cpp/build/libunique_ptr.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libunique_ptr.a");
