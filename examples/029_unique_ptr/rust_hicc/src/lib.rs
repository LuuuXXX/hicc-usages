...
//! 029_unique_ptr: std::unique_ptr 默认 deleter
//!
//! hicc 模式：unique_ptr 默认 deleter 时，工厂用 `hicc::make_unique<T, Args&&...>(Args&&...)`
//! 返回类对象本身。**Rust 端 Resource 的 Drop 等价于 C++ 端 unique_ptr 的释放**，
//! 不需要再绑 `consume_resource(std::unique_ptr<T>)` — 该参数触发了 hicc 已知 bug
//! (`make_unique_arg` 未定义)。
//!
//! 为了演示 consume 语义，cpp! 块写一个**返回值版本**的 wrapper，把 consume 内联。
