...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find namespace_nested.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("namespace_nested_hicc");

    // Link the externally-built C++ static library (../cpp/build/libnamespace_nested.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libnamespace_nested.a");
