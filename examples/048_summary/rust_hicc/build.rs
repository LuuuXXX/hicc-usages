...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find summary.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("summary_hicc");

    // Link the externally-built C++ static library (../cpp/build/libsummary.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libsummary.a");
