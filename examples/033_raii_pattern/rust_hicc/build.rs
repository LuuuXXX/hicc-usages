...
fn main() {
    ...
    // .include(cpp_dir) lets the adapter find raii_pattern.h (Deref<Target=cc::Build>).
    hicc_build::Build::new()
        .rust_file("src/lib.rs")
        .include(&cpp_dir)
        .compile("raii_pattern_hicc");

    // Link the externally-built C++ static library (../cpp/build/libraii_pattern.a).
    ...
    println!("cargo::rerun-if-changed=../cpp/build/libraii_pattern.a");
